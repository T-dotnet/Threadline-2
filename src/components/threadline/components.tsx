...
import { Sparkles, ChevronDown, ChevronRight, Calendar, FileText, Share2, Edit3 as EditIcon, Search } from 'lucide-react';
import { BRAND, DIVIDER, TEXT_PRIMARY, TEXT_SECONDARY, TEXT_DISABLED, primaryBtn, outlineBtn, TYPE_SCALE, cardStyle, h1Style, subStyle } from './constants';
...
export function DataTable({ columns, children }: { columns: TableColumn[]; children: React.ReactNode }) {
  return (
            ...
            {columns.map(col => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: col.align === 'right' ? 'flex-end' : 'flex-start', gap: 4 }}>
                  {col.label}
                  {col.sortable && <ChevronDown size={13} />}
                </div>
              </th>
            ))}
          </tr>
...
export interface ListViewLayoutProps {
  ...
  rpp: number;
  setRpp: (v: number) => void;
  totalRows: number;
}

export function ListViewLayout({
  title, subtitle, actionLabel, actionIcon, onAction,
  search, onSearchChange, searchPlaceholder = 'Search...',
  leftControls, children,
  page, setPage, rpp, setRpp, totalRows,
}: ListViewLayoutProps) {
  const start = totalRows === 0 ? 0 : page * rpp + 1;
  const end = Math.min((page + 1) * rpp, totalRows);
  const totalPages = Math.max(1, Math.ceil(totalRows / rpp));

  return (
    <div style={{ padding: '32px 0 64px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 0 32px 0' }}>
        <div>
      ...
      <div style={cardStyle}>
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${DIVIDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
          <div>{leftControls ?? null}</div>
          <div style={{ position: 'relative', width: 320 }}>
            ...
            <input
              ...
              style={{ width: '100%', height: 44, padding: '0 16px 0 40px', border: `1px solid ${DIVIDER}`, borderRadius: 4, fontSize: 14, outline: 'none', fontFamily: "'Poppins', sans-serif", boxSizing: 'border-box' }}
            />
          </div>
        </div>
        ...
        <div style={{ padding: '14px 24px', borderTop: `1px solid ${DIVIDER}`, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 20, fontSize: 13, color: TEXT_SECONDARY }}>
          <span>Rows per page:</span>
          <select
            value={rpp}
            onChange={e => { setRpp(Number(e.target.value)); setPage(0); }}
            style={{ border: `1px solid ${DIVIDER}`, borderRadius: 4, padding: '4px 8px', fontSize: 13, color: TEXT_PRIMARY, cursor: 'pointer', outline: 'none', fontFamily: "'Poppins', sans-serif" }}
          >
            {[5, 10, 25].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          <span style={{ color: TEXT_PRIMARY }}>{totalRows === 0 ? '0' : `${start}–${end}`} of {totalRows}</span>
          <div style={{ display: 'flex', gap: 4 }}>
            <button
              onClick={() => setPage(Math.max(0, page - 1))}
              disabled={page === 0}
              style={{ background: 'none', border: 'none', cursor: page === 0 ? 'default' : 'pointer', color: page === 0 ? TEXT_DISABLED : TEXT_PRIMARY, padding: '4px 10px', fontSize: 18, lineHeight: 1 }}
            >
              ‹
            </button>
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: page >= totalPages - 1 ? 'default' : 'pointer', color: page >= totalPages - 1 ? TEXT_DISABLED : TEXT_PRIMARY, padding: '4px 10px', fontSize: 18, lineHeight: 1 }}
            >
              ›
            </button>
          </div>
