...
export function ClientListScreen({ onSelectClient }: { onSelectClient: (id: string) => void }) {
  ...
  return (
    <div style={{ padding: "32px 0 64px" }}>
      ...
      <div style={{ ...cardHeaderStyle, borderBottom: "none", padding: "0 0 32px 0" }}>
        <div>
          <h1 style={h1Style}>Clients</h1>
          <p style={subStyle}>Manage your diagnostic registry and client relationships.</p>
        </div>
        <button style={primaryBtn}><AddIcon size={18} /> Add Client</button>
      </div>

      <div style={cardStyle}>
        <div style={{ ...cardHeaderStyle, justifyContent: "space-between", gap: 16 }}>
          <SimpleDropdown 
            label="Clinician"
            ...
            {clients.map((c, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: i < clients.length - 1 ? `1px solid ${DIVIDER}` : "none", transition: "background 0.2s" }} className="hover:bg-gray-50">
                  ...
                  <td style={{ padding: "16px" }}>
                    {status !== 'idle' ? <StatusBadge status={status} /> : <span style={{ color: TEXT_DISABLED, fontSize: 13 }}>—</span>}
                  </td>
                  ...
                  <td style={{ padding: "16px" }}>
                    <span style={{
                      display: "inline-block", padding: "4px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600,
                      border: `1px solid ${c.consent ? "#bbf7d0" : "#fecaca"}`,
                      color: c.consent ? "#166534" : "#991b1b",
                      background: c.consent ? "#f0fdf4" : "#fef2f2",
