...
import { Plus, X, ChevronDown, MessageSquare, ClipboardCheck, Clock, ArrowRight, Calendar, FileText, Users, AlertCircle } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { TEXT_PRIMARY, TEXT_SECONDARY, DIVIDER, BRAND, primaryBtn, outlineBtn, cardStyle, cardHeaderStyle, cardContentStyle, h1Style, subStyle, TYPE_SCALE } from "./constants";
    ...
    <div style={{ 
      ...
      <div style={{ fontSize: 14, lineHeight: 1.5 }}>
        ...
        <button 
          ...
          style={{ background: "none", border: "none", color: "#b45309", fontWeight: 600, padding: 0, marginLeft: 8, cursor: "pointer", textDecoration: "underline" }}
        >
          View details
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        {showPrompt ? (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            style={{ ...cardStyle, padding: 40, textAlign: "center", maxWidth: 700, margin: "40px auto" }}
          >
            ...
            <h2 style={{ ...TYPE_SCALE.HeadingHero, marginBottom: 12 }}>Clinical Hypothesis</h2>
            <p style={{ fontSize: 16, color: TEXT_SECONDARY, marginBottom: 32, lineHeight: 1.6 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <button 
                ...
                style={{ ...primaryBtn, width: "100%", height: 52, fontSize: 16, opacity: hypothesisText.length < 20 ? 0.5 : 1 }}
              >
              ...
              <button
                ...
                style={{ ...outlineBtn, width: "100%", height: 52, fontSize: 16, borderColor: DIVIDER, color: TEXT_SECONDARY }}
              >
          ...
          >
            {flags.FEATURE_HYPOTHESIS_FRAMING_PROMPT && hypothesisSubmitted && (
              <div style={{ ...cardStyle, marginBottom: 32, background: "#f8fafc", border: "1px solid #e2e8f0", padding: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, color: "#475569" }}>
                  <ClipboardCheck size={20} />
                  <span style={{ fontWeight: 600, fontSize: 15 }}>Recorded Hypothesis</span>
                </div>
                ...
                {[
                  ...
                  <div key={step.id} style={{ display: "flex", gap: 24 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#f3f4f6", border: `1px solid ${DIVIDER}`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 500, color: TEXT_SECONDARY }}>
                        {step.id}
                      </div>
                    </div>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {step.content.map((item, idx) => (
                          <div key={idx} style={{ border: `1px solid ${DIVIDER}`, borderRadius: 8, padding: 16, display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f9fafb" }}>
              ...
              <div style={{ marginTop: 48, padding: "32px 0", borderTop: `1px solid ${DIVIDER}` }}>
                {isDeferred ? (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    style={{ background: "#f8fafc", border: `1px solid ${BRAND}`, borderRadius: 12, padding: 32 }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 12, color: BRAND, marginBottom: 24 }}>
                      <Clock size={20} />
                      <span style={{ fontWeight: 600, fontSize: 18 }}>Assessment deferred — {deferDate}</span>
                    </div>

                    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                      <div>
                        <label style={{ display: "block", ...TYPE_SCALE.HeadingSmall, marginBottom: 8 }}>What's still needed:</label>
                        <textarea 
                          value={missingInfo}
                      ...
                      <div>
                        <label style={{ display: "block", ...TYPE_SCALE.HeadingSmall, marginBottom: 8 }}>Next step:</label>
                        <select 
                          value={nextStep}
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end" }}>
                        <button 
                          ...
                          style={{ ...outlineBtn, padding: "0 24px", height: 40, fontSize: 13 }}
                        >
                          Revise Decision
                        </button>
                      </div>
                  ...
                  <div style={{ display: "flex", gap: 24 }}>
                    ...
                    <button 
                      onClick={handleDefer}
                      style={{ 
                        ...outlineBtn, flex: 1, height: 56, fontSize: 16, fontWeight: 600, borderRadius: 12,
                        borderColor: "#64748b", color: "#475569", background: "#f8fafc",
                    ...
                    <button 
                      ...
                      style={{ 
                        ...primaryBtn, flex: 1, height: 56, fontSize: 16, fontWeight: 600, borderRadius: 12,
                        display: "flex", alignItems: "center", justifyContent: "center", gap: 10
                      }}
                    >
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 16 }}>
                     <button onClick={handleDefer} style={{ ...outlineBtn, height: 44, px: 24, padding: "0 24px" }}>Defer</button>
                     <button onClick={handleFormulate} style={{ ...primaryBtn, height: 44, px: 24, padding: "0 24px" }}>Formulate</button>
                  </div>
                )}
              </div>
            </div>
